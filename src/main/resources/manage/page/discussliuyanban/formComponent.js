Vue.component('discussliuyanban-form', {
template:`
<template>
    <div>
        <el-dialog
...
methods: {
    ...
    getInfo(){
        http.get(`discussliuyanban/info/${this.id}`).then(res=>{
    ...
    save(){
        ...
        this.$refs.formRef.validate((valid)=>{
            ...
            if(crossUserId&&crossRefId){
                ...
                http.get('discussliuyanban/page',{
                    params:params
                }).then(res=>{
                    if(res.data.data.total>=crossOptNum){
                        return this.$message.error(this.crossTips)
                    }else{
                        http.post(`discussliuyanban/${!this.form.id ? "save" : "update"}`,this.form).then(res=>{
                            if(this.type == 'cross'){
                                //修改跨表数据
